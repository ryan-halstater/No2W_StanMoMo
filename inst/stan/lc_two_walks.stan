...
model {
  ...
  target += dirichlet_lpdf(b2|rep_vector(1, J));  // Prior on beta_x
  target += exponential_lpdf(sigma1 | 0.1);         // Exponential prior for sigma
  target += exponential_lpdf(sigma2 | 0.1); 
