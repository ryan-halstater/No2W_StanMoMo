...
parameters {
  ...
  vector[J] a;                          // alpha_x
  unit_vector[J] b;                        // beta_x, squares sum to 1

  real c;                           // drift term
  vector[T] ks;                   // vector of kappa
...
model {
  ...
  // equivalent to mean(ks) ~ normal(0,0.01)
  sum(ks) ~ normal(0, 0.01 * T);
}
...
generated quantities {
  ...
  int pos2= 1;
  int pos3= 1;
  k_p[1] = c+k[T]+sigma * normal_rng(0,1);   // Simply applying same model for forecasting
